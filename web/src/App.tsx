...
function FileChangesPanel({ records, selectedPhaseId }: { records: FileChangeRecord[]; selectedPhaseId: string | null }) {
  const totalChanges = records.reduce((sum, r) => sum + r.changes.length, 0);
  return <div className="file-changes-panel" data-testid="file-changes-panel"><div className="file-changes-panel-header"><span className="eyebrow">File changes</span><span className="file-changes-panel-count">{totalChanges > 0 ? `${totalChanges} change${totalChanges === 1 ? '' : 's'}` : 'none'}</span></div>{records.length === 0 ? <div className="file-changes-panel-empty"><span className="muted">No file changes observed</span></div> : <ScrollArea className="file-changes-panel-body">{records.map((record) => <div key={record.seq} className={`file-changes-record${record.phase_id === selectedPhaseId ? ' is-selected' : ''}`}><div className="file-changes-record-meta"><span className="file-changes-phase">{record.phase}</span><time>{formatTime(record.ts)}</time></div><ul className="file-changes-items">{record.changes.map((change, index) => <li key={`${change.path}-${index}`} className={`file-change-item is-${change.action}`}><span className="file-change-action">{change.action}</span><span className="file-change-path">{change.path}</span>{change.size !== undefined && <small>{change.size}{change.prev_size !== undefined ? ` ← ${change.prev_size}` : ''} B</small>}</li>)}</ul></div>)}</ScrollArea>}</div>;
}
