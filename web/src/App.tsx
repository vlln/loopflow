...
function RunsWorkspace() {
  ...
  useEffect(() => {
    ...
    void api.run(selectedId).then((value) => {
      setDetail(value);
      if (value.status === 'waiting_input') void api.interventions(value.run_id).then((page) => setInterventions(page.items)).catch((cause) => setError(messageOf(cause))); else setInterventions([]);
  ...
  return <section className={`workspace runs-workspace ${selectedCall ? 'has-call' : 'no-call'}`} data-testid="runs-workspace" data-mobile-pane={mobilePane}>
    ...
    <section className="panel run-detail-panel">
      {!detail ? <Empty title="Select a Run" detail="Phase execution and events appear here." /> : <>
        ...
        {detail.status === 'waiting_input' && <InterventionPanel runId={detail.run_id} items={interventions} onAnswered={async () => { await loadRuns(); setDetail(await api.run(detail.run_id)); setInterventions([]); }} onError={setError} />}
