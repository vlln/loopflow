...
```
~/.loopflow/loops/<name>/
├── loop.md                # 必选。声明式定义：frontmatter + body
├── workflow.py            # 编排逻辑
...
Loop 的声明式定义文件，YAML frontmatter + Markdown body。**必选**——没有 loop.md 的目录不被视为 loop。
...
|------|------|------|
| name | 是 | loop 唯一标识 |
| description | 是 | 简短描述。含冒号时用引号包裹：`"Demo: test loop"` |
| triggers | 否 | 触发声明列表。无调度需求时写 `- type: manual` |
| triggers[].type | 是 | `manual` / `cron` / `watch` |
| triggers[].schedule | 否 | cron 表达式（type=cron 时必填） |
| resources | 否 | 需要的资源类型列表 |
...
Body 是 Markdown，内容自由但建议包含：目的、流程、权限边界、升级条件。Body 是人类和 Agent 的文档入口。
...
**注意：`name` 和 `description` 不在 `meta` 中定义。** 身份信息只在 `loop.md` 的 frontmatter 中声明，避免双重真相。
