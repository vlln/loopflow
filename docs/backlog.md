...
| BL-011 | npm audit 既有失败修复 | brace-expansion 经 @vitest/coverage-v8 链 5 high，中断 mr-gate 链 | 0081 mr-gate 验证时发现 | done | 0.25.0 |
...
| BL-013 | 版本号单源化 | 版本号双写（pyproject.toml + src/loopflow/__init__.py）易不同步，0.21.0 release 冒烟时暴露 | 0.21.0 复盘 | done | 0.25.0 |
...
| BL-051 | WebUI 二进制文件预览（图片/PDF） | file changes 面板点击 PDF/PNG 等二进制文件时显示 `file_not_previewable`（`preview()` 拒绝含 `\x00` 的文件）。需新增 raw 文件端点流式返回二进制内容（带正确 Content-Type），preview API 对图片/PDF 返回 `encoding:"raw"` + `raw_url`，前端用 `<img>`/`<iframe>` 渲染。支持格式：png/jpg/gif/svg/webp/bmp/ico/pdf，上限 50 MiB | bio-reproducer 远端 run 文件预览失败 2026-07-29 | done | 0.27.0 |
| BL-052 | loop run 默认追加 prompt 参数 | 所有 loop 运行应有一个 `--prompt`/`--append-prompt` 参数，允许用户输入任意字符串追加到每个 agent 调用的 prompt 末尾（或作为 system prompt 补充）。用于调试和临时干预，不必修改 agent 定义或 workflow | 用户 2026-07-29 | done | 0.27.0 |
| BL-053 | New Run 目录选择器支持创建文件夹 | WebUI New Run 对话框的 Browse 目录选择器目前只能浏览和选择已有目录，不可创建新文件夹。应加"新建文件夹"按钮 | 用户 2026-07-29 | candidate | — |
| BL-054 | New Run 自动填写 loop 声明的 args keys | WebUI New Run 对话框的 Arguments 编辑器应自动预填 loop 的 `loop.md` 中 `args:` 声明的参数名（key），用户只需填值。当前完全空白，用户需手动输入参数名 | 用户 2026-07-29 | done | 0.27.0 |
| BL-055 | ADR-0052 Web 契约传导与 strict 覆盖补齐 | active Spec/AC/Interface 仍要求已删除的 PhaseGraph、phase occurrence、declared_phases/from_phase；Web strict 语义审查另发现有效 AC 只有局部测试。先对齐为 AgentGraph/call_id/label，再补完整测试与必要产品行为 | 0109 TEST_INFRA 语义审查 2026-07-29 | done | 0.27.0 |
| BL-056 | ADR-0008 部署决策与实际发布流程不符 | ADR-0008 声明发布到 PyPI，但历史所有版本（0.24~0.27）实际均按本地型发布（release/* → main + tag），无 PyPI publish 配置或执行记录；PyPI 上 loopflow 是他人的包。应修订 ADR-0008 为本地型发布，或补齐真实 PyPI 发布流程 | 0.27.0 RELEASE 发现 2026-07-30 | planned | 0.28.0 |
| BL-057 | demo loop 覆盖 agent 生成二进制 + 主动 intervention 端到端 | 现有 loops 无一让 agent 生成/复制 png/pdf 到工作目录（bio-reproducer 只写 metrics.json）；manual-approval/bio-reproducer 有 intervene 但无文件生成。需造一个 mock backend（`--mock bash`）的 demo loop：某阶段用 bash 生成 `chart.png`/`report.pdf`，另一阶段 `intervene()` 请求批准，配 Playwright/system 测试跑该 loop 验证 WebUI 二进制预览（AC-033）+ intervention 应答（AC-023）的端到端，不付费可入 CI | 0.27.1 deep-research 实跑发现 2026-08-02 | planned | 0.28.0 |
| BL-058 | SYSTEM_TEST 真实 agent 端到端黑盒盲区 | 0108 SYSTEM_TEST 对 AC-033（二进制预览）和 AC-023（intervention）只用了 fixture/mock 集成覆盖，没有真实 agent 跑出 png/主动 intervene 的端到端黑盒；parallel 事件串线（BL 并入 0.27.1）正是这种盲区漏过的真实缺陷。SYSTEM_TEST 规范"不调付费 Agent"导致盲区，应用 mock backend demo loop（见 BL-057）补真实链路黑盒，而非只跑 fixture | 0.27.1 SYSTEM_TEST 复盘 2026-08-02 | planned | 0.28.0 |
| BL-060 | hotfix 流程未走 Plan 容器 | hotfix/0.27.1（web 静态 404 + 事件串线）直接以 fix commit 合入 main+develop，未按热修复流程创建最小修复 Plan（docs/plans/ 无对应容器），跳过轻量 DESIGN/Report 留档。热修复是快速通道但不应无痕——修复内容、验收证据应落在 Plan/Report。下次 hotfix 需补此流程 | 0.27.1 进度检查复盘 2026-08-03 | candidate | — |
| BL-059 | 子进程级测试 flaky | `test_mock_acp_support::test_mock_context_prefix_only_first_prompt` 等用子进程起 mock backend 的测试在 CI 偶发失败（本地 3 次全过，重跑即过），属子进程启动/时序竞态。应让这类测试确定性（同步等待就绪信号替代 sleep、或缩短竞态窗口），避免 CI 噪声掩盖真实回归 | 0.27.1 sync PR #33 首跑失败重跑过 2026-08-03 | planned | 0.28.0 |
