...
| BL-056 | ADR-0008 部署决策与实际发布流程不符 | ADR-0008 声明发布到 PyPI，但历史所有版本（0.24~0.27）实际均按本地型发布（release/* → main + tag），无 PyPI publish 配置或执行记录；PyPI 上 loopflow 是他人的包。应修订 ADR-0008 为本地型发布，或补齐真实 PyPI 发布流程 | 0.27.0 RELEASE 发现 2026-07-30 | candidate | — |
...
| BL-059 | 子进程级测试 flaky | `test_mock_acp_support::test_mock_context_prefix_only_first_prompt` 等用子进程起 mock backend 的测试在 CI 偶发失败（本地 3 次全过，重跑即过），属子进程启动/时序竞态。应让这类测试确定性（同步等待就绪信号替代 sleep、或缩短竞态窗口），避免 CI 噪声掩盖真实回归 | 0.27.1 sync PR #33 首跑失败重跑过 2026-08-03 | candidate | — |
