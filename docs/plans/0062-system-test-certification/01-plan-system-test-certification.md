...
5. 如 MR gate 失败，先记录失败层级和判断；只有局部 bug 且用户同意时才创建后续修复 Plan。
...
| AC manifest | 全局 scenarios 通过，无 planned | Report: PASS |
| Recovery manifest | recovery scenarios 通过，无 planned | Report: PASS |
