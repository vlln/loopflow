...
```jsonl
{"phase":"采集","phase_id":"phase-1","ts":"2026-07-23T12:00:05Z","changes":[{"path":"data/raw.json","action":"created","size":1024}]}
{"phase":"处理","phase_id":"phase-2","ts":"2026-07-23T12:00:10Z","changes":[{"path":"data/raw.json","action":"modified","size":2048,"prev_size":1024},{"path":"data/clean.json","action":"created","size":512}]}
```
...
| event_id | 严格递增，SSE 游标 | 无 event_id，不通过 SSE 推送 |
| 读取方式 | application service 结构化查询 | application service 独立查询 |

**file_changes.jsonl 不参与 ADR-0034 的 v2 事件信封、不参与 ADR-0036 的确定性重放契约、不参与 SSE 事件流。** WebUI 通过独立的 REST 端点查询文件变化，不通过 SSE 推送。
...
- 优点：单一事件流，SSE 自动推送。
- 缺点：污染业务事件契约；文件变化不是业务语义，不应参与确定性重放；违反 ADR-0034 的事件信封设计意图；SSE 推送大列表影响实时事件延迟。
...
本 ADR 约束 runtime 的 phase 边界快照采集、file_changes.jsonl 的写入，以及 application service 的文件变化查询读模型。

- runtime 在 `phase()` 调用时执行快照和 diff，追加到 `file_changes.jsonl`；
- application service 提供按 run_id 和 phase_id 查询文件变化的读模型；
- Web adapter 暴露独立 REST 端点，不通过 SSE 推送；
...
| legacy Run 兼容 | 无 file_changes.jsonl 的旧 Run | WebUI 显示空状态，不报错 |
