...
|--------|---------|------|------|------------|
| 序号重放正确性 | 编写 `MinimalResumeRuntime`，模拟 3 个 agent 调用，在第 2 个后崩溃，resume | 可行 | 第 1-2 个缓存命中，第 3 个执行，结果与完整运行一致。核心机制 <50 行代码 | spike/0004-resume-mechanism |
| while 循环 resume | 模拟 5 次迭代的 while 循环，在第 3 次崩溃，resume | 可行 | 前 3 次缓存命中，后 2 次执行，while 循环正确完成 5 次迭代后退出 | spike/0004-resume-mechanism |
| parallel 内部 resume | 模拟 3 个并发 agent，A 和 B 已完成，C 未执行，resume | 可行 | A 和 B 缓存命中，C 执行，全部完成后返回 | spike/0004-resume-mechanism |
| 损坏缓存处理 | 写入非法 JSON 作为缓存文件，resume 时检查 | 可行 | JSONDecodeError 被捕获，视为未完成，重新执行并覆盖损坏文件 | spike/0004-resume-mechanism |
