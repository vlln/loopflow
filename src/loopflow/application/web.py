...
class WebApplication:
    ...
    def _respond_intervention_items(self, run_id: str, responses: list[dict[str, Any]]) -> dict[str, Any]:
        # Shared with the CLI respond channels (ADR-0056 §1/§2): the HTTP
        # layer only parses arguments, all semantics live in respond_and_recover
        from loopflow.application.respond import respond_and_recover

        return respond_and_recover(self.runs, self.executor, run_id, responses)
